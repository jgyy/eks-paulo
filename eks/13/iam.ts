...
class IAM extends Parameters {
  ...
  BackendAppsCacheAccess = () => {
    const oidc = `oidc.eks.ap-southeast-1.amazonaws.com/id/${this.EndPoint()}`;
          ...
          Statement: [{
            ...
            Condition: {
              StringEquals: {
                ...
                [`${oidc}:sub`]: 'system:serviceaccount:backend-apps:cache-access',
              },
            },
            Effect: 'Allow',
            Principal: {
              Federated: `arn:aws:iam::${this.AccountId}:oidc-provider/${oidc}`,
            },
          }],
          Version: '2012-10-17',
        }),
        managedPolicyArns: [
          this.AmazonDynamoDBReadOnlyAccess(),
          this.AmazonElastiCacheFullAccess(),
        ],
      },
    );
  };
  ...
  BackendAppsS3Reader = () => {
    ...
    return new aws.iam.Role(
      'BackendAppsS3Reader',
      {
        assumeRolePolicy: JSON.stringify({
          Statement: [{
            ...
            Condition: {
              StringEquals: {
                [`${oidc}:aud`]: 'sts.amazonaws.com',
                [`${oidc}:sub`]: 'system:serviceaccount:backend-apps:s3-reader',
              },
            },
            Effect: 'Allow',
            Principal: {
        ...
        managedPolicyArns: [this.AmazonS3ReadOnlyAccess()],
      },
    );
  };
  ...
  PolicyCertManagerChangeSet = () => new aws.iam.Policy(
    'PolicyCertManagerChangeSet',
    {
      name: `${this.StackName}-PolicyCertManagerChangeSet`,
      policy: JSON.stringify({
        Statement: [{
  ...
  PolicyCertManagerGetChange = () => new aws.iam.Policy(
    'PolicyCertManagerGetChange',
    {
      name: `${this.StackName}-PolicyCertManagerGetChange`,
      policy: JSON.stringify({
        Statement: [{
  ...
  PolicyCertManagerHostedZones = () => new aws.iam.Policy(
    'PolicyCertManagerHostedZones',
    {
      name: `${this.StackName}-PolicyCertManagerHostedZones`,
      policy: JSON.stringify({
        Statement: [{
          Action: [
