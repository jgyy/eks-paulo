...
class Parameters {
  ...
  getCallerIdentity = aws.getCallerIdentity;
  ...
  StackName = 'cluster-13';

  AccountId = () => pulumi.output(
    this.getCallerIdentity({}),
  ).accountId;

  AmazonDynamoDBReadOnlyAccess = () => pulumi.output(
    this.getPolicy({
  ...
  AmazonElastiCacheFullAccess = () => pulumi.output(
    this.getPolicy({
      name: 'AmazonElastiCacheFullAccess',
    }),
  ).arn;

  AmazonS3ReadOnlyAccess = () => pulumi.output(
    this.getPolicy({
  ...
  EndPoint = () => this.resource.controlPlane.endpoint.apply((e) => e.slice(8, 40));
